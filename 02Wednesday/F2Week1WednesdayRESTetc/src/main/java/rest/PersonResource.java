...
public class PersonResource {
    ...
    public String addPerson(String person) {
        Address add = FACADE.addAddress(new Address("Cake", "CAAAKE", 123123));
        Person pdto = GSON.fromJson(person, Person.class);
        Person p = FACADE.addPerson(pdto.getfName(), pdto.getlName(), pdto.getPhone());
        //p.setAddress(FACADE.addAddress(new Address("Cake", "CAAAKE", 123123)));
        FACADE.updateAddress(p, add);
    ...
    public String update(@PathParam("id") int id, String p) throws PersonNotFoundException {
        ...
        System.out.println(ppl.getAddress());
        System.out.println(ppl.getCreated());
        System.out.println(ppl.getLastEdited());
        System.out.println(ppl.getfName());
        System.out.println(ppl.getlName());
        FACADE.updateAddress(ppl, FACADE.getAddressByID(id));
        if (FACADE.getPerson(id) == null) {
            throw new PersonNotFoundException("Could not update person with id " + id + " - Person not found");
        }
        return GSON.toJson((FACADE.editPerson(ppl, id)));
    }
